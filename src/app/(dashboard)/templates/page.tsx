...
import {
  ...
  CheckCircle, CheckCircle2, Clock, Search, LayoutGrid,
} from "lucide-react";
...
function TemplateCard({ template, onEdit, onDelete, onOpen }: CardProps) {
  ...
  /**
   * "Published" and "usable" are not the same thing: a template with no fields
   * can be published, and visit creation still refuses to offer it because
   * there would be nothing to fill in. Saying only "نشط" on such a card leaves
   * someone hunting for it in the visit modal, so the card says which it is.
   */
  const isPublished = template.status === "active";
  const isUsable    = isPublished && template.field_count > 0;
  ...
  return (
    ...
    >
      ...
      <div className="p-5">
        ...
        {/* Published but empty: explain why it will not appear in visits. */}
        {isPublished && template.field_count === 0 && (
          <p className="mb-3 text-[11.5px] text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-2.5 py-1.5">
            {t("templates.notUsableEmpty")}
          </p>
        )}
        ...
        {updatedLabel && (
          <p className="mt-2 text-[10.5px] text-ink-400">
            {t("templates.lastUpdated")}: {updatedLabel}
          </p>
        )}
      </div>
    </div>
