...
import {
  ...
  ToggleLeft, ToggleRight, AlertCircle, AlertTriangle, Info,
...
//
// IMPORTANT (domain boundary): a schedule is a recurring ASSIGNMENT RULE.
// It is a different table and a different concept from a visit, and it does
// NOT generate visit rows — there is no schedule_id on `visits`, no trigger
// and no job that materialises occurrences. Everything rendered here is a
// planned assignment, never an executable visit. The banner below states
// that to the user so the distinction never depends on DB knowledge.
//
// This panel is the former /schedule page body, lifted verbatim so all CRUD,
// filters and both views keep working. Its modals are still imported from
// schedule/_components (underscore folders are excluded from routing, not
// from imports), so no schedule code had to move.
// ─────────────────────────────────────────────────────────────────────────────
...
function EmptyState({ onAdd, t }: { onAdd: () => void; t: TranslationFn }) {
  return (
    <div className="flex flex-col items-center justify-center py-14 text-center">
      ...
      <p className="text-[12.5px] text-ink-400 mb-5 max-w-xs">
...
function TableHead({ t }: { t: TranslationFn }) {
  ...
  return (
    <tr className="border-b border-ink-100 bg-ink-50/60">
      <th className={th}>{t("schedule.merch")}</th>
      <th className={th}>{t("schedule.place")}</th>
      <th className={th}>{t("schedule.day")}</th>
      <th className={th}>{t("schedule.startTime")}</th>
      <th className={th}>{t("schedule.frequency")}</th>
    ...
    <tr className={cn(
      ...
      <td className="px-4 py-3.5">
        <div className="flex items-center gap-1.5">
          ...
          {/* A rule with no checklist still generates valid visits, but the
              merchandiser gets no template — surfaced, never silent. */}
          {!schedule.template_id && (
            <span
              title={t("schedule.templateNoneWarning")}
              className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-amber-50 text-amber-700 text-[11px] font-semibold"
            >
              <AlertTriangle className="w-2.5 h-2.5" />
              {t("schedule.noTemplateBadge")}
            </span>
          )}
        </div>
      </td>
...
export function RecurringSchedulesPanel() {
  ...
  return (
    <>
      {/* Recurring rules now materialise into real visits (migration 019 +
          the daily generator), so the copy states what actually happens. */}
      <div className="flex items-start gap-2.5 rounded-xl border border-emerald-100 bg-emerald-50/60 px-3.5 py-2.5 mb-4">
        <Info className="w-4 h-4 text-emerald-600 shrink-0 mt-0.5" />
        <p className="text-[12.5px] text-emerald-900 leading-snug">
          {t("visits.recurringNote")}
        </p>
      </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 h-10 px-4 rounded-xl bg-brand-500 hover:bg-brand-600 text-white text-[13px] font-semibold shadow-pop transition-all"
          >
            <Plus className="w-4 h-4" />
            {t("schedule.add")}
          </button>
      ...
      <div className="bg-white rounded-2xl border border-ink-100 shadow-soft overflow-hidden">
        {/* Filters (schedule-specific: day / merch / place / search) */}
        {!isLoading && !isError && totalCount > 0 && (
        ...
        {isLoading && (
          view === "week" ? (
            <div className="p-4 overflow-x-auto"><SkeletonWeekly /></div>
          ) : (
      ...
      {/* Modals — unchanged schedule CRUD */}
      {showCreate && <ScheduleModal onClose={() => setShowCreate(false)} />}
      {editTarget && (
