...
import { useDeleteChain } from "@/hooks/use-chains";
...
export function DeleteModal({ chain, onClose }: DeleteModalProps) {
  const { t, locale } = useTranslation();
  const deleteChain   = useDeleteChain();

  const chainName = locale === "ar" ? chain.name_ar : chain.name_en;
    ...
    >
      ...
      >
        ...
        <div className="px-6 pt-8 pb-6 text-center">
          ...
          >
            {chainName}
            <span className="opacity-70">({chain.code})</span>
          </div>

          <p className="text-[12.5px] text-ink-500 leading-relaxed mt-2">
            {t("chains.confirmDeleteMsg")}
          </p>
        </div>
        ...
        <div className="flex gap-3 px-6 pb-6">
          ...
          >
            ...
            {deleteChain.isPending ? t("common.loading") : t("common.delete")}
          </button>
        </div>
      </div>
    </div>
