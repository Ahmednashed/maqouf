...
function ChartPanel({
  ...
  /** Stretch across the whole analytics grid (used by the trend chart). */
  full?:    boolean;
}) {
  return (
    <div className={full ? "sm:col-span-2 xl:col-span-3" : undefined}>
...
function NoData({ t }: { t: TranslationFn }) {
  return (
    <div className="h-[140px] flex flex-col items-center justify-center gap-1.5">
      {/* Mini illustration: ghosted bars */}
      <div className="flex items-end gap-1.5 h-8" aria-hidden="true">
        {[12, 20, 9, 25, 15].map((h, i) => (
          <span
            key={i}
            className="w-2 rounded-t bg-ink-100"
            style={{ height: `${h}px` }}
          />
        ))}
        <LineChart className="w-3.5 h-3.5 text-ink-200 mb-1 ms-1" />
      </div>
      <p className="text-[11.5px] font-semibold text-ink-400">{t("dashboard.chart.noData")}</p>
      <Link
        href="/visits"
        className="inline-flex items-center gap-1 text-[11px] font-semibold text-brand-500 hover:text-brand-600 hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-400 rounded"
      >
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3" dir="ltr">
        ...
        {/* Row C · 30-day trend (full width) */}
        <ChartPanel title={t("dashboard.section.trend")} full>
